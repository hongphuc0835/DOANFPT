...
namespace paymentservice.Payments
{
	public sealed class PaypalClient
	{
		...
		// Phương thức hoàn tiền từ PayPal
		public async Task<RefundOrderResponse> RefundOrder(string orderId, string refundAmount)
		{
			try
			{
				...
				// Bước 2: Lấy thông tin đơn hàng từ PayPal
				var httpResponse = await httpClient.GetAsync($"{BaseUrl}/v2/checkout/orders/{orderId}");
				if (!httpResponse.IsSuccessStatusCode)
				{
					var errorContent = await httpResponse.Content.ReadAsStringAsync();
					throw new Exception($"Error retrieving order details: {errorContent}");
				}

				var orderResponse = JsonSerializer.Deserialize<OrderDetailsResponse>(await httpResponse.Content.ReadAsStringAsync());

				// Bước 3: Lấy Capture ID từ đơn hàng
				var captureId = orderResponse.purchase_units[0].payments.captures[0].id;

				// Bước 4: Tạo nội dung request cho việc hoàn tiền
				var refundContent = new StringContent(
					JsonSerializer.Serialize(new { amount = new { value = refundAmount, currency_code = "USD" } }),
					Encoding.UTF8,
					"application/json"
				);

				// Bước 5: Gửi request hoàn tiền
				var refundResponse = await httpClient.PostAsync($"{BaseUrl}/v2/payments/captures/{captureId}/refund", refundContent);
				if (!refundResponse.IsSuccessStatusCode)
				{
					var errorContent = await refundResponse.Content.ReadAsStringAsync();
					throw new Exception($"Error occurred while processing refund: {errorContent}");
				}

				var refundJsonResponse = await refundResponse.Content.ReadAsStringAsync();
				return JsonSerializer.Deserialize<RefundOrderResponse>(refundJsonResponse);
			}
			catch (Exception ex)
			{
