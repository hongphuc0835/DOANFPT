    ...
    const fetchBookings = async () => {
      try {
        ...
        if (response.data) {
          ...
          const bookingsToday = response.data.filter((booking) => booking.bookingDate.split("T")[0] === today); // Lọc các bookings trong ngày hôm nay
          setTotalBookings(bookingsToday.length); // Cập nhật tổng số bookings
        } else {
    ...
    <Grid item xs={12} sm={6} md={3}>
      ...
      >
        <Box sx={{ display: "flex", alignItems: "center" }}>
          <Link to="/booking_today" style={{ textDecoration: "none", color: "inherit" }}>
            <Box
              sx={{
