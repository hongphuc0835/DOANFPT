  ...
  const fetchAllBookings = useCallback(async () => {
    try {
      ...
      if (cachedBookings) {
        ...
        const bookingsToday = JSON.parse(cachedBookings).filter((booking) => booking.bookingDate.split("T")[0] === today);
        const sortedData = bookingsToday.sort((a, b) => new Date(b.bookingDate) - new Date(a.bookingDate));
      ...
      const bookingsToday = response.data.filter((booking) => booking.bookingDate.split("T")[0] === today);
      const sortedData = bookingsToday.sort((a, b) => new Date(b.bookingDate) - new Date(a.bookingDate));
